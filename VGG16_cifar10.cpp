...
int main()
{
    tvm::runtime::Module mod_dylib = tvm::runtime::Module::LoadFromFile("../tvm_output_lib/VGG16_cifar10_tuned.so");
    DLDevice dev{kDLCPU, 0};
    tvm::runtime::NDArray x = tvm::runtime::NDArray::Empty({1, 3, 32, 32}, DLDataType{kDLFloat, 32, 1}, dev);
    tvm::runtime::NDArray y = tvm::runtime::NDArray::Empty({1, 10}, DLDataType{kDLFloat, 32, 1}, dev);
    ...
    for (const auto &entry : fs::directory_iterator(folderPath))
    {
        ...
        if (std::regex_search(imgPath, match, labelRegex))
        {
            ...
            */
            ...
            float stddev[3] = {0.2023, 0.1994, 0.2010};
